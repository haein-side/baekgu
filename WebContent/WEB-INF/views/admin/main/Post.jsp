	...
	<c:if test="${ !empty sessionScope.loginAdminName }">
		...
		<section id="main-content">
			...
			<div class="row">
				<div class="col-lg-12">
					<section class="panel">
						<header class="panel-heading"> 공고 리스트 </header>
									...
									<c:forEach var="post" items="${ requestScope.postList }">
										<tr>
											...
											<fmt:formatDate value="${now}" pattern="yyyy-MM-dd"
												var="today" />
											<fmt:formatDate value="${ post.endDate }"
												pattern="yyyy-MM-dd" var="endday" />
											<c:choose>
												<c:when test="${today <= endday }">
													<td><c:out value="게시중" /></td>
												</c:when>
												<c:when test="${today >  endday}">
													<td><c:out value="게시종료" /></td>
												</c:when>
											</c:choose>
						...
						<div class="pagingArea" align="center">
							...
							<!-- 검색 폼 -->
							<form id="loginForm"
								action="${ pageContext.servletContext.contextPath }/admin/postSearch"
								method="get">
									...
									<c:choose>
										<c:when test="${ !empty requestScope.searchValue }">
											<select id="searchCondition" name="searchCondition">
												<option value="name"
													<c:if test="${requestScope.searchCondition eq 'name'}">selected</c:if>>기업명</option>
												<option value="code"
													<c:if test="${requestScope.searchCondition eq 'code'}">selected</c:if>>기업코드</option>
												<option value="category"
													<c:if test="${requestScope.searchCondition eq 'category'}">selected</c:if>>기업분류</option>
												<option value="state"
													<c:if test="${requestScope.searchCondition eq 'state'}">selected</c:if>>승인여부</option>
											</select>
