...
/**
 * Servlet implementation class AdminManagerSearchServlet
 */
@WebServlet("/admin/adminsearch")
public class AdminManagerSearchServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		int totalCount = adminservice.adminManagerSerach(searchSelect, searchInput);
		...
		List<AdminDTO>  adminDTO = adminservice.adminSearchSelect(searchSelect, searchInput, pageInfo);
		
		String path = "";
		if (adminDTO != null) {
			path = "/WEB-INF/views/admin/main/Manager.jsp";
			request.setAttribute("adminList", adminDTO);
			request.setAttribute("pageInfo", pageInfo);
			request.setAttribute("searchSelect", searchSelect);
			request.setAttribute("searchInput", searchInput);
		} else {
			path = "/WEB-INF/views/admin/common/errorPage.jsp";
			request.setAttribute("message", "관리자 검색에 실패했습니다. 관련 문의는 개발자에게 문의해주세요.");
		}
